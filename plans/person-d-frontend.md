...
4. `backend/cubist/api/websocket.py` — the Python source of truth that mirrors `events.ts`.
...
### Step 1 — Branch, install, run dev server (20 min)
...
You should see the placeholder `<h1>Cubist</h1>` page on a dark background.

### Step 2 — Build a mock event stream (1 hour)
...
### Step 3 — `useEventStream` hook (45 min)
...
**4a. `StrategistFeed.tsx` (1 hour)** — highest demo value. Renders one card per `strategist.question` event with a category-color badge and a status indicator that flips on the matching `builder.completed` event.

**4b. `EloChart.tsx` (1 hour)** — Recharts `LineChart`. Build an Elo history from `generation.finished` events: `[{gen: 0, elo: 1500}, {gen: 1, elo: 1528}, ...]`. Animated entry on each new point.

**4c. `LiveBoard.tsx` (1.5 hours)** — wraps `react-chessboard`. Find the most recent `game.move`, render its FEN. Show White/Black names and a "thinking..." indicator after 2s of no new move event for that game.

**4d. `Bracket.tsx` (1 hour)** — for the active generation, a 6×6 grid of pairings. Cells fill in W/L/D as `game.finished` events arrive. Champion column highlighted. Final column = total points.

**4e. `GenerationTimeline.tsx` (45 min)** — full history. Each row: gen number, 5 questions, winner, Elo delta, promoted/persisted badge.

### Step 5 — `App.tsx` layout (45 min)
  ...
  return (
    <div className="min-h-screen p-6">
      <header className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">CUBIST</h1>
        <button
...
- [ ] Step 5 layout renders end-to-end with `?mock=1` by **hour 8**.
- [ ] Real WS events drive the dashboard once E ships (~hour 12).
...
- **Pre-record a backup video at hour 21.** The Procfile + Vite + WS pipeline can flake on demo day. A 30-second screen recording is your insurance policy.
